#!/usr/bin/env python3
"""
Script para crear un checkpoint final con el espacio de búsqueda completamente corregido.
"""
...
def create_final_checkpoint():
    """Crear un checkpoint final con el espacio de búsqueda completamente corregido."""

    print("=" * 70)
    print("CREANDO CHECKPOINT FINAL CON ESPACIO DE BÚSQUEDA CORREGIDO")
    print("=" * 70)
    ...
    # Datos de trials con espacio de búsqueda final corregido
    final_trials = {
        "0": {
            "number": 0,
            "state": "COMPLETE",
            ...
            "params": {
                ...
                "p_drop_conv": 0.2,  # Dropout rate (solo 0.2 y 0.5)
                "p_drop_fc": 0.5,  # Dropout rate FC (solo 0.2 y 0.5)
    ...
    print(f"📊 Creando checkpoint final con {len(final_trials)} trials")
    print("🔧 Espacio de búsqueda final corregido:")
    print("   - Batch size: [16, 32, 64] ✅")
    print("   - Dropout rate: [0.2, 0.5] ✅")
    ...
    with open(trials_file, "w") as f:
        json.dump(final_trials, f, indent=2)

    # Encontrar mejor trial
    best_trial = None
    best_value = -1
    for trial_data in final_trials.values():
    ...
    best_params_data = {
        ...
        "timestamp": "2025-10-27T22:45:00.000000",
    }
    ...
    completed_trials = len(
        [t for t in final_trials.values() if t["state"] == "COMPLETE"]
    )
    progress_data = {
    ...
    print(f"   - Trials: {len(final_trials)}")
    ...
    print("\n🚀 ¡Checkpoint final creado! Espacio de búsqueda completamente alineado")
    print("=" * 70)
